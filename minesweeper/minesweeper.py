...
class Sentence():
    ...
    def known_mines(self):
        ...
        """
        if(len(self.cells)==self.count and self.count):
    ...
    def mark_mine(self, cell):
        ...
        if(cell in self.cells):
            self.cells.discard(cell) # remove the cell that is known to be mine
            if(len(self.cells)):
                self.count -= 1 #decreasing count of mines as we removed one mine
        else:
            return
    ...
    def mark_safe(self, cell):
        ...
        """
        if(cell in self.cells):
            # we know that this cell is safe
            # so we are removing it from the sentence
            self.cells.discard(cell)
            # since count represente no. of mines in the sentence 
            # we need not to make any change to count as we did not deal with mines
        else:
            return
